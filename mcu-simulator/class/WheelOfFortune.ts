...
export class Whell{
    ...
    show(ctx:CanvasRenderingContext2D){
        ...
        for(let i=360/10;i<=360;i+=360/10){
            ...
            ctx.lineTo(this.x+Math.cos((i+this.angle)*Math.PI/180)*this.radius,this.y+Math.sin((i+this.angle)*Math.PI/180)*this.radius);
            ctx.stroke();
            console.log(i)
        }
       this. angle+=this.angularVelocity/this.framesPerSecond;
        setTimeout(window.requestAnimationFrame.bind(null,this.show.bind(this,ctx)),1000/this.framesPerSecond);
